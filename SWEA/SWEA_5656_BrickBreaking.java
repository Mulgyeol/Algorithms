...
public class SWEA_5656_BrickBreaking {
	static class Point{
		int x;
		int y;
		int size;
	...
	private static void downBricks(int[][] newMap) {
		for (int y = 0; y < W; y++) {
			int x = H-1;
			while(x>=0 && newMap[x][y]==0) --x;
			if(x<0) continue; // 모두 빈칸이면 내릴 것이 없으므로 옆열로 넘어간다.
			
			// 내릴것이 있다!
			x = H-1;
			while(x>0) { // 자신의 위치에 내리기!
				if(newMap[x][y] == 0) { // 빈칸이면
					//직전부터 위쪽 탐색하며 가장 가까운 벽돌 찾기
					int nx = x-1;
					
					//만약 내릴 벽돌을 찾지 못했다면 맨위에 0이 있을거고, 맨 위랑 바꿔도 상관없다.
					while(nx>0 && newMap[nx][y]==0) -- nx;
					newMap[x][y] = newMap[nx][y];
					newMap[nx][y] = 0;
				}
				--x; //위로 올라간다.
			}
